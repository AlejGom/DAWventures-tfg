      ...
      @foreach ($comments as $comment)
        <div class="bg-white border rounded-lg shadow p-6">
          <div class="flex items-center gap-x-4">
            <img src="../{{ $comment->user->profile_image }}" alt="{{ $comment->user->name }}" class="h-10 w-10 rounded-full bg-gray-50">
...
<div id="imageModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black bg-opacity-75 p-4">
  <div class="relative">
    ...
    <button class="absolute top-2 right-2 text-white text-2xl font-bold" onclick="closeModal()">&times;</button>
  </div>
</div>
